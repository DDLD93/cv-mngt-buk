...
export default function BasicModal({ insertUser }) {
  ...
  async function addUser() {
    try {
      ...
      let response = await fetch(`${config.baseUrl}/api/v1/user/register`, {
        method: "POST",
        headers: {
  ...
  async function getUserAdmin() {
    try {
      const response = await fetch(`${config.baseUrl}/api/v1/user/staff admin`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  useEffect(() => {
    getUserAdmin().
      then(r => {
        ...
        setmanagerList(r?.data)
        console.log("r.payload", r.data)
      }).
