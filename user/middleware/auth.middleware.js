...
const config = require("../config");
  ...
  function  admin (req, res, next) {
       try {
           ...
           if (!token) return res.status(403).send("Request has no token");
    
           const decoded = jwt.verify(token, config.secrets.jwt);
           if (!decoded) return res.status(403).send("Access denied.");
           req.user = decoded;
           if (decoded.userType != "admin") return res.status(403).send("Restricted route");
           next();
       } catch (error) {
    ...
    function staffAdmin (req, res, next) {
       try {
           const token = req.header("x-auth-token");
           if (!token) return res.status(403).send("Request has no token");
...
module.exports ={admin, staffAdmin, staff}
